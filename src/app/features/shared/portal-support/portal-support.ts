...
export class PortalSupport implements OnInit {
  ...
  submit():void{if(this.form.invalid||this.saving()){this.form.markAllAsTouched();return;}const v=this.form.getRawValue();const u=this.auth.currentUser();if(!u)return;this.saving.set(true);this.error.set('');this.service.create({customerId:u.roleName==='Client'?u.profileId:null,email:u.email,subject:v.subject.trim(),description:v.description.trim(),priority:v.priority}).subscribe({next:r=>{this.saving.set(false);this.message.set(r.message);this.form.reset({subject:'',description:'',priority:'Medium'});this.load();},error:e=>{this.saving.set(false);this.error.set(e?.error?.message??'No fue posible crear el ticket.');}});}
}
